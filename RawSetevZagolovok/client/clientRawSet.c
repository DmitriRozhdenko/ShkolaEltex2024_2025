...
int main()
{
		...
		int i;
		socklen_t len;
		struct sockaddr_in addr; //создаем структуру для клиента для семействf адресов AF_INET
		//struct udphdr *udp;
		char buf[64];  			// буфер для передачи
		char rcv[92];			//приемный буфер
		memset(buf, 0, sizeof(buf));// обнулить структуру
	...
	while(1)
     {
		recvfrom(sock, rcv, 28+64,0,(struct sockaddr *)&addr,&len); //трансп(8)+сетевой(20)+приемные данные
